...
const getUserData = async (req, res) => {
    ...
    try {
        ...
        if(!reqUserData) return res.status(404).json({ msg: "cannot find user data" });
        const {username,createDate,mostFavorites} = reqUserData; 
        const userSongData = await userSongs.findOne({owner:id})
        if(!userSongData) return res.status(404).json({ msg: "cannot find user songs data" });
        const numberOfSongs = userSongData.songs.length;
        res.status(200).json({msg: {username,createDate,mostFavorites,numberOfSongs}});
    } catch (err) {
